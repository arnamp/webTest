...
word_Motion = "Motion Sensor(IRZ1ZT)"
                    ...
                    for line_number, line in enumerate(file, start=1):  
                        if word_Temp in line:
                            with open('data.text') as fs:
                                ...
                                insert_data(company_data,device,bit_data_Temphumid1,Info1)
                                insert_data_tempgrp(device,round(decode_bit_temp1,2))
                            ...
                            with open('data.text') as fs:
                                ...
                                Temp2 = round(decode_bit_temp2,2)
                                Humid2 = decode_bit_humid2
                                Info2 = 'Temp : {} Celcius , Humid : {} RH%'.format(Temp2,Humid2)
                                insert_data(company_data,device,bit_data_Temphumid2,Info2)
                                insert_data_tempgrp2(device,round(decode_bit_temp2,2))
                        elif word_Motion in line:
                            with open('data_Motion.json', 'w') as json_file_4:
                                json.dump([json_get], json_file_4, indent=4)
        except:
            print("Error")
